...
function repeat() {
    let CounterValue = 0; //CounterValue set to 0, we could use null, but no
    let Feline = ""; // define something called Feline, it is empty
    for (let i = 0; i < 3; i++) { // for loop repeating 3 times starting at zero
        CounterValue++; //CounterValue added by 1
        Feline += "cat "; //repeat Feline by cat, meaning it would be "cat cat cat"
        document.getElementById("catduplicator").innerHTML = Feline; //put that into innerhtml
    }
    if (CounterValue == 3) { //so if the CounterValue is 3, obviously
        document.getElementById("catduplicator2").innerHTML = "Victory!"; //display we are in victory
    }
    else {
        return //else, return for safety if someone tries to do something odd with the inputs
    }
}